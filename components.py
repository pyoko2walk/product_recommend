...
def display_product(result):
    ...
    # 「在庫状況」が「残りわずか」の場合は黄色で「残りわずか」、 「なし」の場合は赤色で「なし」を表示
    if product['stock_status'] == ct.STOCK_STATUS_LOW_STOCK:
        st.warning(ct.STOCK_STATUS_LOW_STOCK_ALERT, icon=ct.WARNING_ICON)
    elif product['stock_status'] == ct.STOCK_STATUS_OUT_OF_STOCK:
        st.error(ct.STOCK_STATUS_OUT_OF_STOCK_ALERT, icon=ct.ERROR_ICON)
